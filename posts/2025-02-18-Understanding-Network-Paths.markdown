...
The MTU discovery tests (Table 2) revealed critical limitations in network packet handling. While packets up to 1000 bytes transmitted successfully with acceptable round-trip times, attempts to send larger packets encountered fragmentation requirements at 1472 bytes. This threshold represents a practical MTU limit in our test environment, likely due to encapsulation overhead and security policy requirements. The increase in both average RTT and standard deviation with larger packet sizes suggests that network path complexity impacts larger packets more significantly.
